    ...
    fn worker_logit(
        ...
        let overlap_credit_decay =
            if request.track_prefill_tokens && weights.overlap_score_credit_decay > 0.0 {
                let active_prefill_tokens = request.active_prefill_tokens_for(worker);
...
impl<C: WorkerConfigLike> WorkerSelector<C> for DefaultWorkerSelector {
    fn select_worker(
        ...
        if let Some(worker) = pinned_worker {
            ...
            let min_active_prefill_tokens = request.active_prefill_tokens_for(worker);
        ...
        let min_active_prefill_tokens =
            if request.track_prefill_tokens && weights.overlap_score_credit_decay > 0.0 {
                candidate_workers
                    .iter()
                    .map(|worker| request.active_prefill_tokens_for(*worker))
                    .min()
                    .unwrap_or(0)
            } else {
                0
...
mod tests {
    ...
    /// Quantifies the production DeepSeek V4 Flash finding: with an 11K-token
    /// prompt (44 blocks of 256), credit=1.0 and prefill_load_scale=1.5, a
    /// full-overlap cache owner is preferred only while its decode-load excess
    /// stays under scale*overlap + overlap = 2.5*43 ~= 108 blocks (~2.5
    /// requests). Beyond that the selector degrades to least-loaded routing.
    #[test]
    fn test_full_overlap_owner_loses_beyond_load_headroom() {
        use crate::test_utils::SimpleWorkerConfig;

        let block_size = 256u32;
        let isl = 11_264usize; // 44 blocks
        let overlap_blocks = 43usize;
        let owner = WorkerWithDpRank::from_worker_id(0);
        let cold = WorkerWithDpRank::from_worker_id(1);
        let workers = HashMap::from([
            (owner.worker_id, SimpleWorkerConfig::default()),
        ...
        // Affinity advantage: cold pays 1.5*44 prefill + 44 new decode blocks;
        // owner pays 1.5*1 prefill + 1 new decode block => headroom 107.5.
        let run = |owner_decode_excess: usize| -> WorkerWithDpRank {
            let (tx, _rx) = tokio::sync::oneshot::channel();
            let mut request = SchedulingRequest {
                ...
                block_hashes: None,
                isl_tokens: isl,
                tier_overlap_blocks: crate::scheduling::TierOverlapBlocks::default(),
                effective_overlap_blocks: HashMap::new(),
            ...
            let cached_tokens = overlap_blocks * block_size as usize;
            request.tier_overlap_blocks.device.insert(owner, overlap_blocks);
            request.effective_cached_tokens.insert(owner, cached_tokens);
            // Potential decode blocks: owner adds only 1 uncached block on top
            // of its excess; cold adds the full 44.
            request.decode_blocks.insert(owner, owner_decode_excess + 1);
            request.decode_blocks.insert(cold, 44);
            request.track_prefill_tokens = true;
            // Zero backlog on both: projected = own uncached delta only.
            request.prefill_tokens.insert(owner, isl - cached_tokens);
            request.prefill_tokens.insert(cold, isl);
            selector
                .select_worker(&workers, &request, block_size)
                .unwrap()
                .worker
        ...
        // Owner logit: 1.5*(44-43) + excess + 1; cold logit: 1.5*44 + 44 = 110.
        // Owner wins while excess < 107.5.
        assert_eq!(run(0), owner, "idle owner must win");
        assert_eq!(run(100), owner, "owner within headroom must win");
        assert_eq!(
            run(110),
            cold,
            "owner beyond ~2.5 requests of excess load loses despite full overlap"
        );
    }
    ...
    fn test_worker_logit_preserves_prefill_accounting_edges() {
        ...
        // 16 tokens of worker prefill backlog + 0 uncached (cached >= isl).
        request.prefill_tokens.insert(worker, 16);
        request.decode_blocks.insert(worker, 5);
        ...
        // raw = (16 backlog+uncached) + 96 cached = 112 tokens = 7 blocks;
        // adjusted = 7 - 6 overlap; logit = 2.0 * 1 + 5 decode = 7.
        assert_eq!(
    ...
    fn test_overlap_credit_decay_can_prefer_less_loaded_cold_worker() {
        ...
        // Warm worker carries 48 tokens of prefill backlog (+0 uncached);
        // cold worker is absent from the map, so it projects the full ISL.
        request.prefill_tokens.insert(warm_worker, 48);
