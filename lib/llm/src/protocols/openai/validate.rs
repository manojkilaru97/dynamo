...
pub fn validate_no_unsupported_fields(
    ...
    validate_no_unsupported_fields_except(unsupported_fields, &[])
}

pub fn validate_no_unsupported_fields_except(
    unsupported_fields: &std::collections::HashMap<String, serde_json::Value>,
    allowed_fields: &[&str],
) -> Result<(), anyhow::Error> {
...
pub fn validate_response_format(
    ...
    match fmt {
        ...
        ResponseFormat::JsonSchema { json_schema } => {
            ...
            {
                anyhow::bail!(
                    "`response_format.json_schema.schema=false` is unsatisfiable and is not supported"
                );
            }
            Ok(())
        }
...
pub fn validate_structured_outputs(
    ...
    if count > 1 {
        anyhow::bail!(
            "Only one structured_outputs constraint may be set at a time (`json`, `regex`, `choice`, `grammar`, or `json_object`)"
        );
    }
    if count == 0 {
        anyhow::bail!(
            "structured_outputs requires one constraint (`json`, `regex`, `choice`, `grammar`, or `json_object`)"
        );
    }
...
pub fn validate_top_p(top_p: Option<f32>) -> Result<(), anyhow::Error> {
    ...
    {
        anyhow::bail!("Top_p must be in [{}, {}], got {}", MIN_TOP_P, MAX_TOP_P, p);
    }
    Ok(())
}
