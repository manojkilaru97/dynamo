...
impl ResponseStreamConverter {
    ...
    pub fn process_chunk(
        ...
        for choice in &chunk.inner.choices {
            ...
            if super::should_emit_reasoning_output(&self.params)
        ...
        if self.reasoning_started {
            ...
            let item_done =
                ResponseStreamEvent::ResponseOutputItemDone(ResponseOutputItemDoneEvent {
                    sequence_number: self.next_seq(),
                    output_index: self.reasoning_output_index,
                    item: self.reasoning_item(OutputStatus::Completed),
                });
            events.push(self.make_sse_event(&item_done));
        }
    ...
    pub fn completed_response_for_logging(&self) -> Response {
        self.make_response(Status::Completed, self.completed_output())
    }

    pub fn completed_response_payload_for_logging(&self) -> serde_json::Value {
        let mut value = serde_json::to_value(self.completed_response_for_logging())
    ...
    fn completed_output(&self) -> Vec<OutputItem> {
        ...
        if self.reasoning_started {
            output.push(self.reasoning_item(OutputStatus::Completed));
        }
        if self.message_started {
    ...
    fn reasoning_item(&self, status: OutputStatus) -> OutputItem {
