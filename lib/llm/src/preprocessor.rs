...
use crate::protocols::{
    ...
    openai::{
        ...
        chat_completions::{
            ...
            first_complete_json_value, jail::JailedStream,
        },
...
impl OpenAIPreprocessor {
    ...
    fn reasoning_stream_modes(&self, request: &NvCreateChatCompletionRequest) -> (bool, bool) {
        ...
        // Nemotron reasons before the guided tool JSON; keep parsing reasoning.
        let forced_json_skips_reasoning = tool_choice_forces_guided_json
        ...
        S: Stream<Item = Annotated<NvCreateChatCompletionStreamResponse>> + Send + 'static,
    {
        ...
        let transformed_stream: Pin<Box<dyn Stream<Item = _> + Send>> = if should_jail {
            Box::pin(Self::apply_tool_calling_jail_with_usage_defer(
            ...
            async move {
                ...
                if let Some(response) = inner.response_stream.next().await {
                    ...
                    if inner.finish_reason_sent && !inner.usage_chunk_sent {
                        ...
                        let annotated_usage = Self::create_usage_annotation(
                            inner.response_generator.as_ref(),
                            inner.trace_tokens_enabled,
                        );
    ...
    where
        S: Stream<Item = Annotated<NvCreateChatCompletionStreamResponse>> + Send + 'static,
    {
        Self::apply_tool_calling_jail_with_usage_defer(
            tool_call_parser,
            tool_choice,
            tool_definitions,
            false,
    ...
    pub fn apply_tool_calling_jail_with_usage_defer<S>(
        tool_call_parser: Option<String>,
        tool_choice: Option<dynamo_protocols::types::ChatCompletionToolChoiceOption>,
        tool_definitions: Option<Vec<dynamo_parsers::tool_calling::ToolDefinition>>,
        ...
        S: Stream<Item = Annotated<NvCreateChatCompletionStreamResponse>> + Send + 'static,
    {
        ...
        builder = builder.defer_terminal_until_usage(defer_terminal_until_usage);
