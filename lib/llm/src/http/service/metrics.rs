...
pub struct Metrics {
    ...
    component_requests_total: IntCounterVec,
...
pub struct ResponseMetricCollector {
    ...
    // we track if prompt tokens have been counted to keep cumulative counters exact
...
impl Metrics {
    ...
    pub fn new() -> Self {
        ...
        let component_requests_total = IntCounterVec::new(
            Opts::new(
                "dynamo_component_requests_total",
                "Compatibility counter for total frontend component requests processed",
            ),
            &["model", "endpoint", "request_type", "status", "error_type"],
        )
        .unwrap();
        ...
        let prompt_tokens_total = IntCounterVec::new(
            Opts::new("prompt_tokens_total", "Total prompt/input tokens processed"),
            &["model"],
        )
        .unwrap();
    ...
    fn init_unified_request_series(&self, model: &str, endpoint: &Endpoint) {
        for request_type in [RequestType::Unary, RequestType::Stream] {
            ...
            for (status, error_type) in [
                ...
                self.component_requests_total.with_label_values(&[
                    model,
                    endpoint.as_str(),
                    request_type.as_str(),
                    status.as_str(),
    ...
    pub fn register(&self, registry: &Registry) -> Result<(), prometheus::Error> {
        ...
        registry.register(Box::new(self.component_requests_total.clone()))?;
...
impl InflightGuard {
    fn new(
        ...
        metrics.init_unified_request_series(&model, &endpoint);
...
impl ResponseMetricCollector {
    ...
    pub fn observe_response_metrics(
        ...
        let num_tokens = if num_tokens > 0 {
            num_tokens
        } else {
            output_tokens.saturating_sub(self.counted_output_tokens)
        };
        if num_tokens == 0 {
            return;
        }
...
pub fn process_response_and_observe_metrics<T>(
    ...
    if let Ok(Some(metrics)) = LLMMetricAnnotation::from_annotation(annotated) {
        ...
        response_collector.observe_response_metrics(
