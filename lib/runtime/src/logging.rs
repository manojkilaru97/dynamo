    ...
    if jsonl_logging_enabled() {
        ...
        if otlp_exporter_enabled() {
            ...
            let otel_logs_layer = OpenTelemetryTracingBridge::new(&logger_provider)
                .with_filter(otel_logs_filter_layer);
...
fn json_payload_to_any_value(value: serde_json::Value) -> AnyValue {
    if json_value_contains_null(&value)
        && let Ok(payload) = serde_json::to_string(&value)
    {
...
///
/// This bypasses tracing's scalar field model so nested `payload` and `headers`
/// survive as OTLP maps instead of JSON strings.
